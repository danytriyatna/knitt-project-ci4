...
$(document).ready(function () {
    ...
    let dtList = new Tabulator("#dt-list", {
        columns: [
                ...
                cellClick: function(e, cell) {
                    ...
                    if (e.target.title === 'delete') {
                        if (confirm("Anda yakin akan menghapus data?")) {
                            window.location.replace(baseUrl + "/master-data/satuan/delete/" + data_row.id);
                        }
                    }else if(e.target.title === 'edit'){
                        inpData.val(data_row.id)
                        inpNamaSatuan.val(data_row.nama_satuan)
            ...
            {
                title: "Nama Satuan", field: "nama_satuan", headerSort: false,
                width: "20%"
            },
        ...
        locale: 'id',    
        layout: 'fitColumns',
        ajaxURL: "/master-data/satuan/list",
        ajaxConfig: "POST",
        sortMode: "remote",
        filterMode: "remote",
    ...
    function simpanData() {
        ...
        if(validation){
            $.ajax({
                type: 'POST',
                url: '/master-data/satuan/simpan',
                data: {
                    dataId : inpData.val(),
                    nama_satuan   : inpNamaSatuan.val(),

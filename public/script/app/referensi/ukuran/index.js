...
$(document).ready(function () {
    ...
    let dtList = new Tabulator("#dt-list", {
                ...
                cellClick: function(e, cell) {
                    ...
                    if (e.target.title === 'delete') {
                        if (confirm("Anda yakin akan menghapus data?")) {
                            window.location.replace(baseUrl + "/master-data/ukuran/delete/" + data_row.id);
                        }
        ...
        locale: 'id',    
        layout: 'fitColumns',
        ajaxURL: "/master-data/ukuran/list",
        ajaxConfig: "POST",
        sortMode: "remote",
        filterMode: "remote",
...
function showHistoryStatus(opex_info_id = 0)
{
    let histStatusModal  = $("#modal-history-status");

    $.ajax({
        type: 'POST',
        url: '/rkap_anper_unit/get_history_status',
        data: {
            opex_info_id: opex_info_id
        },
        dataType: "json",
        beforeSend: function () {
            Swal.fire({
        ...
        success: function (response) {
            let contentHistory = response.content_history;

            var histStatusModalBody = document.getElementById('modal-history-status-body');
            histStatusModalBody.innerHTML = contentHistory;

            Swal.close();
            histStatusModal.modal("show");
        },
        error: function (e) {
            let msg = e.responseJSON.message;
            Swal.close();

...
let inpJmlStok = $('#jml_masuk');
let inpStok = $('#stok'); 
let inpStokAsal = $('#stok_asal'); 
...
dtList.on("rowClick", function(e, row){
    ...
    if(selectKategori.val().length> 0){
        ...
        } else if(selectKategori.val() == 2 || selectKategori.val() == 3  || selectKategori.val() == 9){
            if(selectGudangTujuan.val() == 0){
                return Swal.fire({
...
inpJmlStok.keyup(function (e) {
    if(inpJmlStok.val() != ""){
        ...
        let totalStok = parseFloat(e.target.value) + parseFloat(inpStok.val())
...
function simpanData() {
    ...
    if(selectKategori.val() > 0){
        if(selectKategori.val() == 1){
            ...
            if(inpJmlStok.val() > inpStokAsal.val()){
                return Swal.fire({
                    text: "Gudang Asal tidak memiliki stok yang cukup",
                    icon: 'error',
                    showConfirmButton: false,
    ...
    if(validation){
        $.ajax({
            type: 'POST',
            url: '/trans/incoming-goods/simpan',
            data: {
...
function getLastStock(idBarang,idGudangTujuan,idGudangAsal){
    $.ajax({
        type: 'POST',
        url: '/trans/incoming-goods/last-stock',
        ...
        success: function (response) {
            ...
            if(response.status == true){
                ...
                inpStokAsal.val(response.stokAsal)
                Swal.close();
            }else{
                Swal.fire({
                    text: response.message,

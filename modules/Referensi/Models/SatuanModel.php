...
class SatuanModel extends \App\Models\PrModel
{
    ...
    function getData($id = null, $offset = null, $limit = null, $order = null, $filters = null, $params = null)
    {
        ...
        $builder->select("uk.id, uk.nama_satuan, uk.keterangan");

        if ($id == null or $id == "") {
            $builder->where('uk.active = 1');
            if (!empty($filters) && is_array($filters) && count($filters) >= 1) {
                $builder->groupStart();
                $builder->where('LOWER(uk.nama_satuan) LIKE', strtolower("%{$filters[0]['value']}%"));

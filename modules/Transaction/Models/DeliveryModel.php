...
class DeliveryModel extends \App\Models\PrModel
{
    ...
    function getData($id = null, $offset = null, $limit = null, $order = null, $filters = null, $params = null)
    {
        ...
        $builder->select("abx.id, abx.tgl_transaksi,  abx.delivery_kode, abx.id_produksi, abx.produksi_kode, abx.id_konsumen, abx.alamat,
                            ...
                            tp.qty, twx.ref_kode as kode_so, tso.deskripsi, abx.id_invoice, tso.ref_po,
                            (
                                tp.qty - COALESCE((
                                    SELECT SUM(abx2.qty) 
    ...
    function getDataProduksi($params = null, $id = null){
        ...
        $builder->select("
            ...
            TRIM(BOTH ' ~ ' FROM
            CASE 
                WHEN tw.tipe_id = 1 AND bs1.id  IS NOT NULL THEN COALESCE(NULLIF(bs1.keterangan,  ''), COALESCE(rw1.keterangan, ''))
                WHEN tw.tipe_id = 2 AND bso1.id IS NOT NULL THEN COALESCE(NULLIF(bso1.keterangan, ''), COALESCE(rw1.keterangan, ''))
                ELSE COALESCE(rw1.keterangan, '') 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs2.id  IS NOT NULL AND NULLIF(bs2.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs2.keterangan
                WHEN tw.tipe_id = 1 AND bs2.id  IS NOT NULL AND NULLIF(bs2.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw2.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso2.id IS NOT NULL AND NULLIF(bso2.keterangan, '') IS NOT NULL THEN ' ~ ' || bso2.keterangan
                WHEN tw.tipe_id = 2 AND bso2.id IS NOT NULL AND NULLIF(bso2.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw2.keterangan, ''), '')
                WHEN rw2.keterangan IS NOT NULL THEN ' ~ ' || rw2.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs3.id  IS NOT NULL AND NULLIF(bs3.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs3.keterangan
                WHEN tw.tipe_id = 1 AND bs3.id  IS NOT NULL AND NULLIF(bs3.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw3.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso3.id IS NOT NULL AND NULLIF(bso3.keterangan, '') IS NOT NULL THEN ' ~ ' || bso3.keterangan
                WHEN tw.tipe_id = 2 AND bso3.id IS NOT NULL AND NULLIF(bso3.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw3.keterangan, ''), '')
                WHEN rw3.keterangan IS NOT NULL THEN ' ~ ' || rw3.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs4.id  IS NOT NULL AND NULLIF(bs4.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs4.keterangan
                WHEN tw.tipe_id = 1 AND bs4.id  IS NOT NULL AND NULLIF(bs4.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw4.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso4.id IS NOT NULL AND NULLIF(bso4.keterangan, '') IS NOT NULL THEN ' ~ ' || bso4.keterangan
                WHEN tw.tipe_id = 2 AND bso4.id IS NOT NULL AND NULLIF(bso4.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw4.keterangan, ''), '')
                WHEN rw4.keterangan IS NOT NULL THEN ' ~ ' || rw4.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs5.id  IS NOT NULL AND NULLIF(bs5.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs5.keterangan
                WHEN tw.tipe_id = 1 AND bs5.id  IS NOT NULL AND NULLIF(bs5.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw5.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso5.id IS NOT NULL AND NULLIF(bso5.keterangan, '') IS NOT NULL THEN ' ~ ' || bso5.keterangan
                WHEN tw.tipe_id = 2 AND bso5.id IS NOT NULL AND NULLIF(bso5.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw5.keterangan, ''), '')
                WHEN rw5.keterangan IS NOT NULL THEN ' ~ ' || rw5.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs6.id  IS NOT NULL AND NULLIF(bs6.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs6.keterangan
                WHEN tw.tipe_id = 1 AND bs6.id  IS NOT NULL AND NULLIF(bs6.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw6.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso6.id IS NOT NULL AND NULLIF(bso6.keterangan, '') IS NOT NULL THEN ' ~ ' || bso6.keterangan
                WHEN tw.tipe_id = 2 AND bso6.id IS NOT NULL AND NULLIF(bso6.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw6.keterangan, ''), '')
                WHEN rw6.keterangan IS NOT NULL THEN ' ~ ' || rw6.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs7.id  IS NOT NULL AND NULLIF(bs7.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs7.keterangan
                WHEN tw.tipe_id = 1 AND bs7.id  IS NOT NULL AND NULLIF(bs7.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw7.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso7.id IS NOT NULL AND NULLIF(bso7.keterangan, '') IS NOT NULL THEN ' ~ ' || bso7.keterangan
                WHEN tw.tipe_id = 2 AND bso7.id IS NOT NULL AND NULLIF(bso7.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw7.keterangan, ''), '')
                WHEN rw7.keterangan IS NOT NULL THEN ' ~ ' || rw7.keterangan ELSE '' 
            END ||
            CASE 
                WHEN tw.tipe_id = 1 AND bs8.id  IS NOT NULL AND NULLIF(bs8.keterangan,  '') IS NOT NULL THEN ' ~ ' || bs8.keterangan
                WHEN tw.tipe_id = 1 AND bs8.id  IS NOT NULL AND NULLIF(bs8.keterangan,  '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw8.keterangan, ''), '')
                WHEN tw.tipe_id = 2 AND bso8.id IS NOT NULL AND NULLIF(bso8.keterangan, '') IS NOT NULL THEN ' ~ ' || bso8.keterangan
                WHEN tw.tipe_id = 2 AND bso8.id IS NOT NULL AND NULLIF(bso8.keterangan, '') IS NULL     THEN COALESCE(' ~ ' || NULLIF(rw8.keterangan, ''), '')
